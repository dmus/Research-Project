...
%   Detailed explanation goes here
    ...
    while ~stop
        ...
        for t = 1:T
            for h = 1:H
                ...
                for i=1:h
                    ...
                    accelerations(1:2) = model{i}.Apos * s + model{i}.Bpos * U(t+h,:)';
                    accelerations(3) = model{i}.Arot * s + model{i}.Brot * U(t+h,:)';
        ...
        Apos = zeros(size(Apos));
        Bpos = zeros(size(Apos));
        Apos(1,:) = theta(1:3);
        Bpos(1,:) = theta(4:7);
        ...
        model{i+1}.Arot = (1-alpha) * model{i}.Apos + alpha * Arot;
        model{i+1}.Brot = (1-alpha) * model{i}.Bpos + alpha * Brot;
